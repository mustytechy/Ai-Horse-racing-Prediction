...
def load_model_assets():
    # Use absolute paths so the cloud server can find files
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    
    # Load model using joblib (required for cloud stability)
...
if uploaded_file:
    ...
    with st.expander("🧠 Inside the AI's Brain: See What Factors Matter Most", expanded=False):
        ...
        fig_brain.update_layout(xaxis_title="Influence on Final Prediction Score (%)", yaxis_title="", showlegend=False, height=340, margin=dict(l=10, r=40, t=10, b=10))
